...
fn run_benchmark(args: &CommonArgs) -> Result<BenchmarkReport> {
    ...
    let indexed_kmers = if needs_precount {
        let inserted_kmers = index_fasta(&args.index_fasta, args.kmer_size, args.batch_bases, &mut bloom)?;
...
fn build_filter(bloom_bits: usize, bloom_hashes: u32, false_positive_rate: f64) -> StableBloomFilter {
...
fn query_fasta(path: &Path, k: usize, batch_bases: usize, filter: &StableBloomFilter) -> Result<QueryStats> {
