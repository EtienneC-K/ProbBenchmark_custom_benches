...
BENCHMARK_NAME = "bench_bloom_sizes versus false positives"
...
METRIC_PATTERNS = {
    "queried_kmers": r"queried_kmers\s+([0-9eE+.\-]+)",
    "positive_kmers": r"query_positive_kmers\s+([0-9eE+.\-]+)",
}
...
def run_filter(root: Path, command: list[str]) -> dict[str, float]:
    ...
    metrics["fp"] = metrics["positive_kmers"]/metrics["queried_kmers"]
    print(metrics["fp"])
    return metrics

def build_fastbloom_command(index_file: str, query_file: str, var: int) -> list[str]:
    command = [
...
def build_classic_command(index_file: str, query_file: str, var: int) -> list[str]:
    command = [
...
def build_roaring_command(index_file: str, query_file: str, var: int) -> list[str]:
    command = [
...
def build_bf_rust_command(index_file: str, query_file: str, var: int) -> list[str]:
    command = [
...
def build_bloomfx_command(index_file: str, query_file: str, var:int) -> list[str]:
    command = [
...
def build_bloom_rs_command(index_file: str, query_file: str, var:int) -> list[str]:
    command = [
...
def build_generic_command(index_file: str, query_file: str, var:int) -> list[str]:
    command = [
...
def plot_rows(rows: list[dict[str, object]], output_path: Path) -> None:
    return()
    x_values = [int(row["bloom_bits"]) for row in rows]
    false_positives = [float(row["fp"]) for row in rows]

    fig, axes = plt.subplots(1, 2, figsize=(12, 5), sharex=True)

    axes.plot(x_values, false_positives, marker="o", label="fp rate")
    axes.set_title("False positve rates versus bloom sizes")
    axes.set_xlabel("bf_size")
    axes.set_ylabel("FP rate")
    axes.grid(True, alpha=0.3)
    axes.legend()

    fig.suptitle("bloomybloom benchmark: BF sizes vs FP")
    fig.tight_layout()
...
def main() -> None:
    ...
    for b_bits in BLOOM_BITS_VALUES:
        ...
        metrics_list = [
            run_filter(root, build_classic_command(args.index_file, args.query_file, b_bits))
            for _ in range(REPEATS)
        ]
        metrics = aggregate(metrics_list)
        classic_rows.append({
        ...
        metrics_list = [
            run_filter(root, build_roaring_command(args.index_file, args.query_file, b_bits))
            for _ in range(REPEATS)
        ]
        metrics = aggregate(metrics_list)
        roaring_rows.append({
        ...
        metrics_list = [
            run_filter(root, build_bf_rust_command(args.index_file, args.query_file, b_bits))
            for _ in range(REPEATS)
        ]
        metrics = aggregate(metrics_list)
        bf_rust_rows.append({
        ...
        metrics_list = [
            run_filter(root, build_bloomfx_command(args.index_file, args.query_file, b_bits))
            for _ in range(REPEATS)
        ]
        metrics = aggregate(metrics_list)
        bloomfx_rows.append({
        ...
        metrics_list = [
            run_filter(root, build_bloom_rs_command(args.index_file, args.query_file, b_bits))
            for _ in range(REPEATS)
        ]
        metrics = aggregate(metrics_list)
        ...
        metrics_list = [
            run_filter(root, build_generic_command(args.index_file, args.query_file, b_bits))
            for _ in range(REPEATS)
        ]
        metrics = aggregate(metrics_list)
        generic_rows.append({
